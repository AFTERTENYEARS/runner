...
#pragma mark - 更新图表
- (void)updateChartDatas:(NSArray <LineChartData *> *)data {
...
- (void)createPointLabel {
    ...
    for (int j = 0; j < _lineChartDatas.count; j++) {
        
        LineChartData *chartData = _lineChartDatas[j];
        NSArray <NSValue *> *pointValueArray = _pointValueArrayArray[j];
        if (chartData.showPointLabel) {//显示点
            
            NSInteger item = chartData.startIndex;
            NSMutableArray *pointLabelArray = [NSMutableArray array];
            for (NSInteger i = 0; i < pointValueArray.count; i++) {
                if (i != _verticalLineIndex-1) {
                    NSValue *value = pointValueArray[i];
...
- (void)getYValueMaxAndYValueMin {
    ...
    if (_yLabelTitles) {
        ...
        for (int i = 0; i < _lineChartDatas.count; i++) {
            LineChartData *chartData = _lineChartDatas[i];
...
- (void)calcuateChart {
    ...
    for (int i = 0; i < _lineChartDatas.count; i++) {
        ...
        for (NSInteger item = chartData.startIndex; item < chartData.itemCount+chartData.startIndex; item++) {
            ...
            UIBezierPath *path = [UIBezierPath bezierPath];
            if (chartData.lineChartPointStyle == LineChartPointStyleCircle) {//圆
            ...
            } else if (chartData.lineChartPointStyle == LineChartPointStyleSquare) {//方形
            ...
            } else if (chartData.lineChartPointStyle == LineChartPointStyleTriangle) { //三角形
...
//            } else{
//                startPoint = [pointValueArray[i] CGPointValue];
//            }
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_showCoordinateAxis) {
        ...
        for (int i = 0; i < _yLabelNum; i++) {
            ...
            CGContextAddLineToPoint(ctx, _chartMarginLeft, y);
        }
